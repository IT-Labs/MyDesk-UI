...
describe("Users can select a desk", () => {
  ...
  it("Verify that correct data is returned when office is selected on home employee page", () => {
    ...
    cy.intercept(`${apiEndpoints.employeeOfficeImage}${SKOPJE_OFFICE_ID}`).as(
      "officeImage"
    );
    cy.intercept(`${apiEndpoints.employeeOfficeDesks}${SKOPJE_OFFICE_ID}`).as(
      "officeData"
    );
    homeEmployeePage.selectOffice("IT-Labs Skopje");
    cy.wait("@officeImage")
      .its("response.body")
      .should("contain", "https://i.postimg.cc/90NqbwHd/in-Office-PLAN.jpg");
